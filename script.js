...
function initializeApp() {
    ...
    document.getElementById("stats-description").textContent = 
        `🚀 JitoX users have collectively earned ${currentBalance.toFixed(2)} SOL through AI-powered MEV strategies today! Our advanced algorithms continuously analyze the Solana blockchain for optimal MEV opportunities, including arbitrage, sandwich attacks, and frontrunning. Watch your earnings grow in real-time!`;
}
...
function updateGlobalNexusStats() {
    ...
    document.getElementById("active-users").innerHTML = `
        <div class="stat-header">Active Searchers</div>
        <div class="stat-value">${activeUsers.toLocaleString()}</div>
        <div class="stat-description">Active MEV searchers using JitoX AI</div>
    `;
    
    document.getElementById("ongoing-mev").innerHTML = `
        <div class="stat-header">Active MEV Operations</div>
        <div class="stat-value">${ongoingMEV.toLocaleString()}</div>
        <div class="stat-description">AI-detected MEV opportunities</div>
    `;
